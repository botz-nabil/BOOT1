...
frnzer.on('chat-update', async (frnazer) => {
		try {
			...
			const ownerNumber = ["6281938835391@s.whatsapp.net"] // Nomor Owner Bre
			const ownerContact = ["6281938835391;","6281938835391"] // Nomor Owner Bre Yg Bentuk Kode Jangan Di Ganti
...
frnzer.sendMessage(from,{contentText:`Hai,${userfernazer}${ucapanWaktu}`,footerText:'LIST SEWA BOT BY NABIL GANZ',buttons:[{buttonId:`${prefix}seminggu`,buttonText:{displayText:'1 MINGGU'},type:1},{buttonId:`${prefix}sebulan`,buttonText:{displayText:'1 BULAN'},type:1}],headerType:'LOCATION',locationMessage:{degreesLatitude:'',degreesLongitude:'',jpegThumbnail:tid,contextInfo:{mentionedJid:[sender]}}},'buttonsMessage') 
break

case 'seminggu':
if (isBanned) return reply (mess.only.banned)
frnzer.sendButton(from,'SEWA BOT 1 MINGGU 5K SILAHKAN PILIH PEMBAYARAN DI BAWAH INI','Creator: Nabil','DANA️',`${prefix}danaa`,{quoted: faketroli})
break

case 'sebulan':
if (isBanned) return reply (mess.only.banned)
frnzer.sendButton(from,'SEWA BOT 1 BULAN 10k SILAHKAN PILIH PEMBAYARAN DI BAWAH INI','Creator: Nabil','DANA️',`${prefix}danaa`,{quoted: faketroli})
break

case 'danaa':
if (isBanned) return reply (mess.only.banned)
frnzer.sendButton(from,`NODANA ${danaa}`,'Creator: NABIL','DONE',`${ucapanWaktu}`,{quoted: faketroli})
break
/******************************* FERNAZER *******************************/
/******************************* RETURN RANDOM *******************************/
...
addCmd(kodenya, q)
reply("Done Jangan Lupa Subrek NABNAB 01\n\nLink Yt: https://linktr.ee/nabil_firmansyah_ !")
} else {
reply('tag stickenya')
}
break
...
frnzer.sendButton(from,ucapanWaktu,'Creator: NABIL','NEXT➡',`${prefix}doge`,{quoted: faketroli})
break	                
...
frnzer.sendButton(from, teks,'Creator: NABIL','KEMBALI KE MENU↩️',`${prefix}menu`,{quoted: faketroli})
break
case 'doaharian':         							
frnzer.updatePresence(from, Presence.composing) 
reply(mess.wait)
